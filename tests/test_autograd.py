...
class TestAutograd(TestCase):
    ...
    def test_autograd_init(self):
        ...
        torch.ones(2, requires_grad=True, device="accinte").sum().backward()
    ...
    def test_autograd_backward(self):
        """Test backward propagation on accinte device"""
        x = torch.randn(2, 3, device="accinte", requires_grad=True)
        y = torch.randn(3, 2, device="accinte", requires_grad=True)
        z = torch.mm(x, y)
        loss = z.sum()
        loss.backward()
