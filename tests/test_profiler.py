...
class TestProfiler(TestCase):
    """Test PyTorch profiler integration with AccInte backend."""

    @skipIfTorchDynamo()
    def test_profiler_basic(self):
        """Test basic profiler functionality with AccInte device."""
        with autograd_profile(use_device="accinte") as prof:
            x = torch.randn(10, 10, device="accinte")
            y = torch.randn(10, 10, device="accinte")
            x + y

        events = prof.function_events
        self.assertGreater(len(events), 0)

        # Check that we have AccInte operations recorded
    ...
    def test_profiler_with_record_function(self):
        ...
        with autograd_profile(use_device="accinte") as prof:
            with record_function("accinte_custom_operation"):
                x = torch.randn(10, 10, device="accinte")
                y = torch.randn(10, 10, device="accinte")
                x @ y
        ...
        events = prof.function_events
        event_names = [e.name for e in events]
        self.assertTrue(any("accinte_custom_operation" in name for name in event_names))

    @skipIfTorchDynamo()
    def test_profiler_multiple_streams(self):
        """Test profiler with multiple AccInte streams."""
        stream1 = torch.Stream(device="accinte")
        stream2 = torch.Stream(device="accinte")
        ...
        with autograd_profile(use_device="accinte") as prof:
            ...
            with stream2:
                z = torch.randn(10, 10, device="accinte")
                z * z
    ...
    def test_profiler_event_synchronization(self):
        ...
        with autograd_profile(use_device="accinte") as prof:
            stream = torch.Stream(device="accinte")
            event = torch.Event(device="accinte", enable_timing=True)
    ...
    def test_profiler_key_averages(self):
        """Test that key_averages works correctly with AccInte operations."""
        with autograd_profile(use_device="accinte") as prof:
            for _ in range(10):
                x = torch.randn(10, 10, device="accinte")
                x + x
    ...
    def test_profiler_with_shapes(self):
        ...
        with autograd_profile(use_device="accinte", record_shapes=True) as prof:
            x = torch.randn(10, 20, device="accinte")
            y = torch.randn(20, 30, device="accinte")
            x @ y
    ...
    def test_profiler_elapsed_time(self):
        ...
        with autograd_profile(use_device="accinte") as prof:
            ...
            with stream:
                x = torch.randn(100, 100, device="accinte")
                y = torch.randn(100, 100, device="accinte")
    ...
    def test_profiler_with_backward_pass(self):
        ...
        with autograd_profile(use_device="accinte", record_shapes=True) as prof:
            x = torch.randn(10, 10, device="accinte", requires_grad=True)
            y = torch.randn(10, 10, device="accinte", requires_grad=True)
            z = (x @ y).sum()
            z.backward()
...
class TestProfilerEdgeCases(TestCase):
    ...
    def test_profiler_exception_handling(self):
        ...
        try:
            with autograd_profile(use_device="accinte") as prof:
                x = torch.randn(10, 10, device="accinte")  # noqa: F841
...
class TestProfilerMultiDevice(TestCase):
    """Test profiler with multiple AccInte devices if available."""

    @skipIfTorchDynamo()
    def test_profiler_single_device(self):
        """Test profiler with explicit device specification."""
        device_count = torch.accinte.device_count()
        if device_count < 1:
            self.skipTest("No AccInte devices available")

        with autograd_profile(use_device="accinte") as prof:
            x = torch.randn(10, 10, device="accinte:0")
            x + x
    ...
    def test_profiler_multiple_devices(self):
        ...
        device_count = torch.accinte.device_count()
        if device_count < 2:
            self.skipTest("Multiple AccInte devices required")

        with autograd_profile(use_device="accinte") as prof:
            x0 = torch.randn(10, 10, device="accinte:0")
            x1 = torch.randn(10, 10, device="accinte:1")
...
class TestProfilerIntegration(TestCase):
    ...
    def test_profiler_with_simple_model(self):
        ...
        model = SimpleModel().to("accinte")
        x = torch.randn(32, 10, device="accinte")
    ...
    def test_profiler_with_training_step(self):
        ...
        x = torch.randn(32, 10, device="accinte")
        target = torch.randn(32, 10, device="accinte")
        ...
        with autograd_profile(use_device="accinte") as prof:
            for i in range(5):
                with record_function(f"batch_{i}"):
                    x = torch.randn(16, 10, device="accinte")
                    model(x)
    ...
    def test_profiler_with_data_transfer(self):
        """Test profiler with CPU-AccInte data transfers."""
        with autograd_profile(use_device="accinte") as prof:
            # CPU tensor
            x_cpu = torch.randn(50, 50)

            # Transfer to AccInte
            with record_function("to_accinte"):
                x_accinte = x_cpu.to("accinte")

            # Compute on AccInte
            with record_function("compute_accinte"):
                y = x_accinte @ x_accinte
        ...
        self.assertTrue(any("to_accinte" in name for name in event_names))
        self.assertTrue(any("compute_accinte" in name for name in event_names))
    ...
    def test_profiler_with_mixed_operations(self):
        ...
        with autograd_profile(use_device="accinte", record_shapes=True) as prof:
            x = torch.randn(20, 20, device="accinte")
