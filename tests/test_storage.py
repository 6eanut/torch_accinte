...
class TestSerialization(TestCase):
    def test_serialization(self):
        ...
        storage = torch.UntypedStorage(4, device=torch.device("accinte"))
        self.assertEqual(torch.serialization.location_tag(storage), "accinte:0")

        storage = torch.UntypedStorage(4, device=torch.device("accinte:0"))
        self.assertEqual(torch.serialization.location_tag(storage), "accinte:0")

        storage_cpu = torch.empty(4, 4).storage()
        storage_accinte = torch.serialization.default_restore_location(
            storage_cpu, "accinte:0"
        )
        self.assertTrue(storage_accinte.is_accinte)

        tensor = torch.empty(3, 3, device="accinte")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            tensor_accinte = torch.load(path)
            self.assertTrue(tensor_accinte.is_accinte)
            self.assertEqual(torch._utils.get_tensor_metadata(tensor_accinte), metadata)

            tensor_cpu = torch.load(path, map_location="cpu")
            self.assertFalse(tensor_cpu.is_accinte)
    ...
    def test_open_device_numpy_serialization(self):
        ...
        expected = torch.tensor(
            [[1, 2, 3], [4, 5, 6]], dtype=torch.float32, device="accinte"
        )
    ...
    def test_open_device_cpu_serialization(self):
        ...
        with unittest.mock.patch.object(torch._C, "_has_storage", return_value=False):
            x = torch.randn(2, 3)
            x_accinte = x.to("accinte")
            sd = {"x": x_accinte}
            rebuild_func = x_accinte._reduce_ex_internal(default_protocol)[0]
            self.assertTrue(
    ...
    def test_serialization_map_location_cpu(self):
        ...
        tensor = torch.randn(3, 3, device="accinte")
    ...
    def test_serialization_map_location_device(self):
        ...
        tensor = torch.randn(3, 3, device="accinte:0")

        with tempfile.TemporaryDirectory() as tmpdir:
            path = os.path.join(tmpdir, "data.pt")
            torch.save(tensor, path)

            loaded_device = torch.load(path, map_location="accinte:1")
    ...
    def test_serialization_storage_location_tag(self):
        ...
        storage = torch.UntypedStorage(4, device=torch.device("accinte:1"))
        self.assertEqual(torch.serialization.location_tag(storage), "accinte:1")
    ...
    def test_serialization_default_restore_location(self):
        ...
        storage_accinte0 = torch.serialization.default_restore_location(
            storage_cpu, "accinte:0"
        )
        self.assertTrue(storage_accinte0.is_accinte)

        storage_accinte1 = torch.serialization.default_restore_location(
            storage_cpu, "accinte:1"
        )
        self.assertTrue(storage_accinte1.is_accinte)
