...
class TestAutocast(TestCase):
    def test_autocast_with_unsupported_type(self):
        ...
        with self.assertWarnsRegex(
            UserWarning,
            "In accinte autocast, but the target dtype is not supported. Disabling autocast.\n"
            "accinte Autocast only supports dtypes of torch.float16, torch.bfloat16 currently.",
        ):
            with torch.autocast(device_type="accinte", dtype=torch.float32):
                _ = torch.ones(10)
    ...
    def test_autocast_operator_not_supported(self):
        ...
        with self.assertRaisesRegex(
            ...
            x = torch.randn(2, 3, device="accinte")
            y = torch.randn(2, 3, device="accinte")
            with torch.autocast(device_type="accinte", dtype=torch.float16):
    ...
    def test_autocast_low_precision(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, 3, device="accinte")
            y = torch.randn(3, 3, device="accinte")
            result = torch.mm(x, y)
            self.assertEqual(result.dtype, torch.float16)

    def test_autocast_fp32(self):
        """Test fp32 operations (asin) in autocast"""
        with torch.amp.autocast(device_type="accinte"):
            x = torch.randn(2, device="accinte", dtype=torch.float16)
            result = torch.asin(x)
            self.assertEqual(result.dtype, torch.float32)

    def test_autocast_default_dtype(self):
        """Test default autocast dtype"""
        accinte_fast_dtype = torch.get_autocast_dtype(device_type="accinte")
        self.assertEqual(accinte_fast_dtype, torch.half)

    def test_autocast_set_dtype(self):
        """Test setting autocast dtype"""
        for dtype in [torch.float16, torch.bfloat16]:
            torch.set_autocast_dtype("accinte", dtype)
            self.assertEqual(torch.get_autocast_dtype("accinte"), dtype)

    def test_autocast_bfloat16(self):
        """Test autocast with bfloat16 dtype"""
        with torch.amp.autocast(device_type="accinte", dtype=torch.bfloat16):
            x = torch.randn(2, 3, device="accinte", dtype=torch.float32)
            y = torch.randn(3, 3, device="accinte", dtype=torch.float32)
            result = torch.mm(x, y)
    ...
    def test_autocast_fp32_with_bfloat16(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.bfloat16):
            x = torch.randn(2, device="accinte", dtype=torch.bfloat16)
            result = torch.asin(x)
            self.assertEqual(result.dtype, torch.float32)
    ...
    def test_autocast_with_requires_grad(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, 3, device="accinte", requires_grad=True)
            y = torch.randn(3, 3, device="accinte", requires_grad=True)
            result = torch.mm(x, y)
            self.assertEqual(result.dtype, torch.float16)
    ...
    def test_autocast_mixed_input_dtypes(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, 3, device="accinte", dtype=torch.float32)
            y = torch.randn(3, 3, device="accinte", dtype=torch.float16)
    ...
    def test_autocast_already_target_dtype(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, 3, device="accinte", dtype=torch.float16)
            y = torch.randn(3, 3, device="accinte", dtype=torch.float16)
            result = torch.mm(x, y)
            self.assertEqual(result.dtype, torch.float16)
    ...
    def test_autocast_combination_operations(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, 3, device="accinte")
            y = torch.randn(3, 3, device="accinte")
            z = torch.randn(2, device="accinte")
    ...
    def test_autocast_disable(self):
        ...
        with torch.amp.autocast(
            device_type="accinte", dtype=torch.float16, enabled=False
        ):
            x = torch.randn(2, 3, device="accinte", dtype=torch.float32)
            y = torch.randn(3, 3, device="accinte", dtype=torch.float32)
            result = torch.mm(x, y)
    ...
    def test_autocast_cache_enabled(self):
        ...
        with torch.amp.autocast(
            device_type="accinte", dtype=torch.float16, cache_enabled=True
        ):
            x = torch.randn(2, 3, device="accinte")
            y = torch.randn(3, 3, device="accinte")
            result1 = torch.mm(x, y)
    ...
    def test_autocast_fp32_operation_with_float32_input(self):
        ...
        with torch.amp.autocast(device_type="accinte", dtype=torch.float16):
            x = torch.randn(2, device="accinte", dtype=torch.float32)
            result = torch.asin(x)
            # asin should output float32
            self.assertEqual(result.dtype, torch.float32)
