...
class TestFactory(TestCase):
    def test_empty(self):
        ...
        x = torch.empty(3, device="accinte")
        self.assertEqual(x.device.type, "accinte")
        self.assertEqual(x.shape, torch.Size([3]))

        x = torch.empty([2, 3, 4, 5], device="accinte", names=["N", "C", "H", "W"])
        self.assertEqual(x.device.type, "accinte")
        self.assertEqual(x.shape, torch.Size([2, 3, 4, 5]))

        with torch._subclasses.fake_tensor.FakeTensorMode():
            x = torch.empty(3, 3, device="accinte")
            y = torch.empty(3, 3, device="accinte:0")
            z = x + y
...
class TestCopy(TestCase):
    def test_copy_same_device(self):
        ...
        a = torch.ones(10, device="accinte").clone()
        self.assertEqual(a, torch.ones(10, device="accinte"))

    def test_cross_device_copy(self):
        """Test copy operation across CPU and accinte"""
        a = torch.rand(10)
        b = a.to(device="accinte").add(2).to(device="cpu")
        self.assertEqual(b, a + 2)

    def test_cross_diff_devices_copy(self):
        """Test copy operation across different accinte devices"""
        a = torch.ones(10, device="accinte:0").to(device="accinte:1").to(device="cpu")
...
class TestOps(TestCase):
    def test_masked_select(self):
        ...
        tensor_accinte = tensor_cpu.to(device="accinte")
        mask = tensor_accinte.gt(0)
        out = torch.masked_select(tensor_accinte, mask)

        self.assertEqual(out, tensor_cpu.masked_select(tensor_cpu.gt(0)))

    def test_expand(self):
        """Test tensor expand operation"""
        x = torch.tensor([[1], [2], [3]], device="accinte")
        y = x.expand(3, 2)
    ...
    def test_resize(self):
        ...
        tensor_accinte = tensor_cpu.accinte()
        self.assertTrue(tensor_accinte.size() == torch.Size([4, 4]))

        storage_accinte = tensor_accinte.storage()
        self.assertTrue(storage_accinte.size() == 16)

        tensor_accinte.resize_(2, 2, 2, 2)
        self.assertTrue(tensor_accinte.size() == torch.Size([2, 2, 2, 2]))
    ...
    def test_printing(self):
        ...
        a = torch.ones(20, device="accinte")
        print(a)
...
class TestSTUB(TestCase):
    def test_backend_dispatchstub(self):
        ...
        x_accinte = x_cpu.to("accinte")

        y_cpu = torch.abs(x_cpu)
        y_accinte = torch.abs(x_accinte)
        self.assertEqual(y_cpu, y_accinte.cpu())

        o_cpu = torch.randn(2, 2, 6, dtype=torch.float32, device="cpu")
        o_accinte = o_cpu.to("accinte")
        # output operand with resize flag is False in TensorIterator.
        torch.abs(x_cpu, out=o_cpu[:, :, 0:6:2])
        torch.abs(x_accinte, out=o_accinte[:, :, 0:6:2])
        self.assertEqual(o_cpu, o_accinte.cpu())
        ...
        torch.abs(x_accinte, out=o_accinte[:, :, 0:6:3])
        self.assertEqual(o_cpu, o_accinte.cpu())
...
class TestQuantization(TestCase):
    def test_quantize(self):
        ...
        x = torch.randn(3, 4, 5, dtype=torch.float32, device="accinte")
        quantized_tensor = torch.quantize_per_tensor(x, 0.1, 10, torch.qint8)
        self.assertEqual(quantized_tensor.device, torch.device("accinte:0"))
...
class TestAutogradFunction(TestCase):
    def test_compile_autograd_function_returns_self(self):
        ...
        in_ref = torch.randn(4, device="accinte", requires_grad=True)
        out_ref = torch.ops.accinte.custom_autograd_fn_returns_self(in_ref)
        out_ref.sum().backward()

        in_test = in_ref.detach().clone().requires_grad_(True)
        # TODO(FFFrog): Need to support inductor for AccInte first.
        out_test = torch.compile(backend="aot_eager")(
            torch.ops.accinte.custom_autograd_fn_returns_self
        )(in_test)
    ...
    def test_compile_autograd_function_aliasing(self):
        ...
        in_ref = torch.randn(4, device="accinte", requires_grad=True)
        out_ref = torch.ops.accinte.custom_autograd_fn_aliasing(in_ref)
        out_ref.sum().backward()

        in_test = in_ref.detach().clone().requires_grad_(True)
        # TODO(FFFrog): Need to support inductor for AccInte first.
        out_test = torch.compile(backend="aot_eager")(
            torch.ops.accinte.custom_autograd_fn_aliasing
        )(in_test)
        out_test.sum().backward()
...
class TestFallback(TestCase):
    def test_scalar_type_fallback(self):
        ...
        x = torch.triu_indices(3, 3, device="accinte")
        self.assertEqual(x_cpu, x)

    def test_tensor_type_fallback(self):
        """Test tensor type fallback to CPU"""
        x = torch.Tensor([[1, 2, 3], [2, 3, 4]]).to("accinte")
        y = torch.Tensor([1, 0, 2]).to("accinte")
        self.assertTrue(x.device.type, "accinte")
        ...
        y = torch.Tensor([1, 0]).long().to("accinte")
        z = x[y, y]
        self.assertEqual(z_cpu, z)
    ...
    def test_tensorlist_type_fallback(self):
        ...
        v_accinte = torch.Tensor([1, 2, 3]).to("accinte")
        # create result tensor located in cpu
        z_cpu = torch.Tensor([2, 4, 6])
        # create tensorlist for foreach_add op
        x = (v_accinte, v_accinte)
        y = (v_accinte, v_accinte)

        # Check that our device is correct.
        self.assertTrue(v_accinte.device.type == "accinte")
        self.assertFalse(v_accinte.is_cpu)
...
class TestSDPA(NNTestCase):
    ...
    def test_fused_sdp_choice_privateuseone(self):
        ...
        q_privateuse1 = q_cpu.to("accinte")
        k_privateuse1 = k_cpu.to("accinte")
        v_privateuse1 = v_cpu.to("accinte")
        assert (
    ...
    def test_scaled_dot_product_fused_attention_overrideable_backward(self):
        ...
        k_privateuse1 = k_cpu.to("accinte")
        v_privateuse1 = v_cpu.to("accinte")
        attn_mask_privateuse1 = attn_mask.to("accinte")
        (
            output,
            logsumexp,
            cum_seq_q,
        ...
        rand_upward_privateuse1 = rand_upward.to("accinte")
...
class TestFactoryExtended(TestCase):
    def test_empty_with_memory_format(self):
        ...
        x = torch.empty(1, 2, 3, 4, device="accinte", memory_format=torch.channels_last)
        self.assertEqual(x.device.type, "accinte")
        self.assertEqual(x.shape, torch.Size([1, 2, 3, 4]))

        x = torch.empty(
            2, 3, 4, device="accinte", memory_format=torch.contiguous_format
        )
        self.assertEqual(x.device.type, "accinte")
    ...
    def test_empty_strided(self):
        ...
        size = (3, 4)
        stride = (4, 1)
        x = torch.empty_strided(size, stride, device="accinte")
        self.assertEqual(x.device.type, "accinte")
    ...
    def test_ones(self):
        ...
        x = torch.ones(3, 4, device="accinte")
        self.assertEqual(x.device.type, "accinte")
    ...
    def test_ones_like(self):
        ...
        x = torch.randn(3, 4, device="accinte")
        y = torch.ones_like(x)
        self.assertEqual(y.device.type, "accinte")
    ...
    def test_full(self):
        ...
        x = torch.full((3, 4), 5.0, device="accinte")
        self.assertEqual(x.device.type, "accinte")
        self.assertEqual(x.shape, torch.Size([3, 4]))
...
class TestCopyExtended(TestCase):
    def test_copy_different_dtypes(self):
        ...
        x = torch.randn(3, 4, dtype=torch.float32, device="accinte")
        y = torch.empty(3, 4, dtype=torch.float64, device="accinte")
        y.copy_(x)
    ...
    def test_copy_non_blocking(self):
        ...
        x = torch.randn(3, 4, device="accinte")
        y = torch.empty(3, 4, device="accinte")
...
class TestOpsExtended(TestCase):
    def test_view(self):
        ...
        x = torch.randn(2, 3, 4, device="accinte")
        y = x.view(6, 4)
        self.assertEqual(y.device.type, "accinte")
    ...
    def test_local_scalar_dense(self):
        ...
        x = torch.tensor([5.0], device="accinte")
        scalar = x.item()
...
class TestSTUBExtended(TestCase):
    def test_abs_contiguous(self):
        ...
        x = torch.randn(2, 3, dtype=torch.float32, device="accinte")
        y = torch.abs(x)
        self.assertEqual(y.device.type, "accinte")
    ...
    def test_abs_out(self):
        ...
        torch.abs(x, out=out)
        self.assertEqual(out.device.type, "accinte")
        self.assertTrue(torch.all(out >= 0))
        self.assertEqual(out, torch.abs(x))


@unittest.skip("Skipping all quantization tests for accinte backend")
class TestQuantizationExtended(TestCase):
    def test_quantize_per_tensor_different_scales(self):
        ...
        self.assertEqual(quantized.device.type, "accinte")
    ...
    def test_dequantize(self):
        ...
        self.assertEqual(dequantized.device.type, "accinte")
...
class TestFallbackExtended(TestCase):
    ...
    def test_fallback_operations(self):
        ...
        x = torch.randn(3, 4, device="accinte")
        y = torch.randn(3, 4, device="accinte")
...
class TestSDPAExtended(NNTestCase):
    ...
    def test_scaled_dot_product_attention_with_dropout(self):
        ...
        self.assertEqual(output.device.type, "accinte")
...
class TestCustomAutogradFunctions(TestCase):
    def test_custom_autograd_fn_returns_self_basic(self):
        ...
        x = torch.randn(4, device="accinte", requires_grad=True)
        y = torch.ops.accinte.custom_autograd_fn_returns_self(x)
    ...
    def test_custom_autograd_fn_aliasing_basic(self):
        ...
        x = torch.randn(4, device="accinte", requires_grad=True)
        y = torch.ops.accinte.custom_autograd_fn_aliasing(x)
    ...
    def test_custom_autograd_fn_returns_self_no_grad(self):
        ...
        x = torch.randn(4, device="accinte", requires_grad=False)
        y = torch.ops.accinte.custom_autograd_fn_returns_self(x)
        self.assertEqual(x, y)
