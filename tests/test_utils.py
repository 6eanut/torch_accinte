...
class TestDLPack(TestCase):
    def test_open_device_dlpack(self):
        """Test DLPack conversion for accinte device"""
        x_in = torch.randn(2, 3).to("accinte")
    ...
    def test_dlpack_different_shapes(self):
        ...
        for shape in shapes:
            x = torch.randn(*shape, device="accinte")
            capsule = torch.utils.dlpack.to_dlpack(x)
            y = torch.from_dlpack(capsule)
    ...
    def test_dlpack_different_dtypes(self):
        ...
        for dtype in dtypes:
            x = torch.randn(2, 3, device="accinte", dtype=dtype)
            capsule = torch.utils.dlpack.to_dlpack(x)
            y = torch.from_dlpack(capsule)
    ...
    def test_dlpack_cross_device(self):
        ...
        capsule = torch.utils.dlpack.to_dlpack(x_accinte)
        y = torch.from_dlpack(capsule)
