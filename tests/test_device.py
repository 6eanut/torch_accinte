...
class TestDevice(TestCase):
    ...
    def test_device_capability(self):
        capability = torch.accelerator.get_device_capability("accinte:0")
    ...
    def test_device_properties(self):
        ...
        device = torch.device("accinte:0")
        self.assertEqual(device.type, "accinte")
    ...
    def test_tensor_device(self):
        ...
        x = torch.randn(2, 3, device="accinte:1")
        self.assertEqual(x.device.type, "accinte")
    ...
    def test_device_poison_fork(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            (
                "Cannot re-initialize AccInte in forked subprocess. "
                "To use AccInte with multiprocessing, you must use the 'spawn' start method"
            ),
        ):
            raise exc
