...
class TestStream(TestCase):
    ...
    def test_stream_create(self):
        ...
        self.assertEqual(stream.device_index, torch.accinte.current_device())
        stream = torch.Stream(device="accinte:1")
        self.assertEqual(stream.device.type, "accinte")
    ...
    def test_stream_context(self):
        ...
        with torch.Stream(device="accinte:1") as stream:
    ...
    def test_stream_context_exception_restore(self):
        ...
        inner_stream = torch.Stream(device="accinte:1")
        try:
            with inner_stream:
    ...
    def test_stream_switch(self):
        ...
        stream1 = torch.Stream(device="accinte:0")
        ...
        stream2 = torch.Stream(device="accinte:1")
        current_stream = torch.accelerator.current_stream()
        self.assertEqual(current_stream, stream1)
    ...
    def test_stream_repr(self):
        ...
        stream = torch.Stream(device="accinte:1")
        self.assertTrue(
            "torch.Stream device_type=accinte, device_index=1" in repr(stream)
        )

    @skipIfTorchDynamo()
    def test_stream_wait_stream(self):
        """Test stream waiting on another stream"""
        stream_1 = torch.Stream(device="accinte:0")
        stream_2 = torch.Stream(device="accinte:1")
        stream_2.wait_stream(stream_1)

    @skipIfTorchDynamo()
    def test_stream_wait_event(self):
        """Test stream waiting on event"""
        s1 = torch.Stream(device="accinte")
        s2 = torch.Stream(device="accinte")
        e = s1.record_event()
        s2.wait_event(e)
    ...
    def test_stream_equality(self):
        ...
        stream1 = torch.Stream(device="accinte:0")
        stream2 = torch.Stream(device="accinte:0")
    ...
    def test_stream_multiple_devices(self):
        ...
        stream0 = torch.Stream(device="accinte:0")
        stream1 = torch.Stream(device="accinte:1")
    ...
    def test_stream_record_event(self):
        ...
        self.assertEqual(event.device.type, "accinte")
        stream.synchronize()
