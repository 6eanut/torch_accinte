...
at::Tensor wrapper_quantize_per_tensor(
    ...
    int64_t zero_point,
    at::ScalarType dtype) {
  return at::native::accinte::quantize_per_tensor(
...
int64_t wrapper__fused_sdp_choice(
    ...
    bool enable_gqa) {
  return at::native::accinte::_fused_sdp_choice(
...
void wrapper_quantize_tensor_per_tensor_affine_stub(
    ...
    double scale,
    int64_t zero_point) {
  at::native::accinte::quantize_tensor_per_tensor_affine_stub(
...
wrapper__scaled_dot_product_fused_attention_overrideable(
    ...
    std::optional<double> scale) {
  return at::native::accinte::_scaled_dot_product_fused_attention_overrideable(
      query,
      key,
      value,
      attn_bias,
...
at::Tensor wrapper_custom_autograd_fn_returns_self(at::Tensor x) {
  return at::native::accinte::custom_autograd_fn_returns_self(x);
}

at::Tensor wrapper_custom_autograd_fn_aliasing(at::Tensor x) {
  return at::native::accinte::custom_autograd_fn_aliasing(x);
}

at::Tensor& wrapper_abs_out(const at::Tensor& self, at::Tensor& out) {
  return at::native::accinte::abs_out(self, out);
}

void wrapper_abs_stub(at::TensorIteratorBase& iter) {
  at::native::accinte::abs_kernel(iter);
}

at::Tensor wrapper_custom_abs(at::Tensor x) {
  return at::native::accinte::custom_abs(x);
}
} // namespace
...
TORCH_LIBRARY_IMPL(accinte, PrivateUse1, m) {
...
TORCH_LIBRARY_FRAGMENT(accinte, m) {
