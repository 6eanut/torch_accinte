...
#include "AccInteGenerator.h"

namespace c10::accinte {
struct ACCINTE_EXPORT AccInteHooksInterface : public at::PrivateUse1HooksInterface {
  AccInteHooksInterface() {};
  ~AccInteHooksInterface() override = default;

  void init() const override {
    // Initialize AccInte runtime if needed
  ...
  bool isBuilt() const override {
    // This extension is compiled as part of the AccInte test extension.
    return true;
  }

  bool isAvailable() const override {
    // Consider AccInte available if there's at least one device reported.
  ...
  // LITERALINCLUDE START: ACCINTE HOOK EXAMPLES
  const at::Generator& getDefaultGenerator(DeviceIndex device_index) const override {
    return getDefaultAccInteGenerator(device_index);
  }
  // LITERALINCLUDE END: ACCINTE HOOK EXAMPLES

  at::Generator getNewGenerator(DeviceIndex device_index) const override {
    return at::make_generator<AccInteGeneratorImpl>(device_index);
  }
};
