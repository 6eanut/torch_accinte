#include "AccInteDeviceAllocator.h"
...
void DeviceMemoryAllocator::free(void* ptr) {
  ...
  if (ret == orSuccess) {
    ...
    if (it != allocation_sizes_.end()) {
      ...
      TORCH_WARN(
          "Successfully freed AccInte memory pointer ",
          ptr,
          " on device ",
          device_index_,
...
AccInteDeviceAllocator g_allocator;

void deleteAccInteMemory(void* ptr) {
  g_allocator.freeMemory(ptr);
}

}

AccInteDeviceAllocator::AccInteDeviceAllocator() {
  std::lock_guard<std::recursive_mutex> lock(mutex_);
  const auto device_count = c10::accinte::device_count();
...
at::DataPtr AccInteDeviceAllocator::allocate(size_t nbytes) {
  int current_device_index = -1;
  auto ret = orGetDevice(&current_device_index);
  TORCH_CHECK(ret == orSuccess, "Failed to get current AccInte device");
  ...
  return {data, data, &deleteAccInteMemory, curr_device};
}

at::DeleterFnPtr AccInteDeviceAllocator::raw_deleter() const {
  return &deleteAccInteMemory;
}

void AccInteDeviceAllocator::copy_data(
    void* dest,
    const void* src,
    std::size_t count) const {
  auto ret = orMemcpy(dest, src, count, orMemcpyDeviceToDevice);
  TORCH_CHECK(
      ret == orSuccess, "Failed to copy ", count, " bytes on accinte device");
}

bool AccInteDeviceAllocator::initialized() {
  std::lock_guard<std::recursive_mutex> lock(mutex_);
  return !device_allocators_.empty();
}

void AccInteDeviceAllocator::freeMemory(void* ptr) {
  if (!ptr) {
    return;
  }
  ...
  if (found_in_map) {
    ...
    // In AccInte's case, orFree returns orErrorUnknown if pointer not in registry
    // which is expected for already-freed memory
    if (ret != orSuccess && ret != orErrorUnknown) {
      TORCH_WARN(
          "orFree failed for untracked AccInte memory pointer ",
          ptr,
          ". Error code: ", ret);
    }
  }
...
c10::CachingDeviceAllocator::DeviceStats AccInteDeviceAllocator::
    getDeviceStats(c10::DeviceIndex device) {
  return device_allocators_[device]->getStats();
}

void AccInteDeviceAllocator::resetAccumulatedStats(c10::DeviceIndex device) {
  device_allocators_[device]->resetAccumulatedStats();
}

void AccInteDeviceAllocator::resetPeakStats(c10::DeviceIndex device) {
  device_allocators_[device]->resetPeakStats();
}

void AccInteDeviceAllocator::emptyCache(MempoolId_t mempool_id) {
  // AccInte doesn't implement caching yet
  // TODO: When caching is implemented, release all free blocks here
}

void AccInteDeviceAllocator::recordStream(
    const DataPtr& ptr,
    c10::Stream stream) {
  // AccInte doesn't track stream usage yet
