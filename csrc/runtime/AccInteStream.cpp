...
std::vector<std::array<
    ...
    c10::accinte::max_compile_time_stream_priorities>>
    streams;
std::deque<
 ...
 * The range 000 to 101 is reserved for stream pools of different priorities and can be expanded as needed. (AccInte currently supports two priorities: 0 and 1)
 *
...
void initGlobalStreamState() {
  ...
  max_stream_priorities = range >= c10::accinte::max_compile_time_stream_priorities
      ? c10::accinte::max_compile_time_stream_priorities
      : range;
}
...
void initSingleDeviceStream(int priority, DeviceIndex device_index, int i) {
  ...
  ACCINTE_CHECK(orStreamCreateWithPriority(&stream, 0, priority));
...
AccInteStream AccInteStreamForId(DeviceIndex device_index, StreamId stream_id) {
  return AccInteStream(
      AccInteStream::UNCHECKED,
      Stream(
          Stream::UNSAFE,
...
AccInteStream getStreamFromPool(const int priority, DeviceIndex device_index) {
  initAccInteStreamsOnce();
  if (device_index == -1) {
  ...
  return AccInteStreamForId(device_index, makeStreamId(id_type, idx));
}

AccInteStream getStreamFromPool(const bool isHighPriority, DeviceIndex device) {
  initAccInteStreamsOnce();
  int priority = isHighPriority ? max_stream_priorities - 1 : 0;
  return getStreamFromPool(priority, device);
}

AccInteStream getStreamFromExternal(
    orStream_t ext_stream,
    DeviceIndex device_index) {
  return AccInteStreamForId(
      device_index, reinterpret_cast<int64_t>(ext_stream));
}

AccInteStream getDefaultAccInteStream(DeviceIndex device_index) {
  initAccInteStreamsOnce();
  if (device_index == -1) {
    device_index = current_device();
  }
...
AccInteStream getCurrentAccInteStream(DeviceIndex device_index) {
  initAccInteStreamsOnce();
  if (device_index == -1) {
    device_index = current_device();
  }
  check_device(device_index);
  return AccInteStreamForId(device_index, current_streams[device_index]);
}

void setCurrentAccInteStream(AccInteStream stream) {
  initAccInteStreamsOnce();
  current_streams[stream.device_index()] = stream.id();
}

std::ostream& operator<<(std::ostream& stream, const AccInteStream& s) {
