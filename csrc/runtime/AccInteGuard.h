...
#include "AccInteEvent.h"
#include "AccInteFunctions.h"
#include "AccInteStream.h"

namespace c10::accinte {

struct AccInteGuardImpl final : public c10::impl::DeviceGuardImplInterface {
  ...
  // LITERALINCLUDE END: ACCINTE GUARD DEVICE MANAGEMENT
   ...
   * By default, AccInte has 2 same devices with the same capability.
   */
  ...
  void synchronizeDevice(const DeviceIndex device_index) const override {
    ACCINTE_CHECK(orDeviceSynchronize());
  }
  // LITERALINCLUDE END: ACCINTE ALL DEVICE GUARD IMPL

  // LITERALINCLUDE START: ACCINTE ALL STREAM GUARD IMPL
  /**
   * Get the current stream for a given device.
   */
  Stream getStream(Device d) const noexcept override {
    return getCurrentAccInteStream(d.index()).unwrap();
  }
  ...
  Stream getDefaultStream(Device d) const override {
    return getDefaultAccInteStream(d.index());
  }
  ...
  Stream exchangeStream(Stream s) const noexcept override {
    const AccInteStream stream(s);
    const auto old_stream = getCurrentAccInteStream(s.device().index());
    setCurrentAccInteStream(stream);
  ...
  // LITERALINCLUDE END: ACCINTE ALL STREAM GUARD IMPL

  // LITERALINCLUDE START: ACCINTE ALL EVENT GUARD IMPL
  /**
  ...
  void destroyEvent(void* event, const DeviceIndex device_index)
    ...
    ACCINTE_CHECK(orEventDestroy(or_event));
  ...
  void record(
    ...
    if (!or_event) {
      ...
      ACCINTE_CHECK(orEventCreateWithFlags(&or_event, or_flag));
    }

    ACCINTE_CHECK(orEventRecord(or_event, or_stream));
    *event = or_event;

    set_device(orig_device);
  }
  // LITERALINCLUDE END: ACCINTE GUARD EVENT RECORD
  ...
  void block(void* event, const Stream& stream) const override {
    ...
    const auto orig_device = current_device();
    set_device(stream.device().index());
    ACCINTE_CHECK(orStreamWaitEvent(or_stream, or_event, 0));
    set_device(orig_device);
  }
  ...
  void synchronizeEvent(void* event) const override {
    ...
    orEvent_t or_event = static_cast<orEvent_t>(event);
    ACCINTE_CHECK(orEventSynchronize(or_event));
  }
  ...
  double elapsedTime(void* event1, void* event2, const DeviceIndex device_index)
    ...
    float time_ms = 0;
    ACCINTE_CHECK(orEventElapsedTime(&time_ms, or_event1, or_event2));

    set_device(orig_device);

    return static_cast<double>(time_ms);
  }
  // LITERALINCLUDE END: ACCINTE ALL EVENT GUARD IMPL
};
