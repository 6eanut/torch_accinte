...
def build_deps():
    ...
    cmake_args = [
        ...
        + os.path.realpath(os.path.join(BASE_DIR, "torch_accinte")),
...
class BuildClean(clean):
    def run(self):
        for i in ["build", "install", "torch_accinte/lib"]:
            dirs = os.path.join(BASE_DIR, i)
            if os.path.exists(dirs) and os.path.isdir(dirs):
                shutil.rmtree(dirs)

        for dirpath, _, filenames in os.walk(os.path.join(BASE_DIR, "torch_accinte")):
...
def main():
    ...
    ext_modules = [
        Extension(
            name="torch_accinte._C",
            sources=["torch_accinte/csrc/stub.c"],
            language="c",
            extra_compile_args=extra_compile_args,
            libraries=["torch_bindings"],
            library_dirs=[os.path.join(BASE_DIR, "torch_accinte/lib")],
    ...
    setup(
        ...
        entry_points={
            "torch.backends": [
                "torch_accinte = torch_accinte:_autoload",
            ],
        },
    )
